...
def home(request):
    ...
    #  get selected college from URL
    selected_college = request.GET.get('college')

    #  base queryset
    if selected_college:
        books_qs = Book.objects.filter(college__icontains=selected_college.strip())
    else:
        books_qs = Book.objects.all()

    # categories (same logic as before)
    categories = books_qs.values_list('category', flat=True).distinct()
   

    for cat in categories:
        prod = books_qs.filter(category=cat)
        allProds.append([prod, range(len(prod))])

    # list of colleges for dropdown
    colleges = Book.objects.values_list('college', flat=True).distinct()
...
def savebook(request):
    ...
    newbook = books.objects.create(sellername=sellername, book_name = book_name, category = category, price= price,image= image,pickuplocation = pickuplocation, college = college, slug= slug)
    try:
        newbook.save()
...
def TrackOrder(request):
    if request.method == "POST":
        ...
        if not order_id:
            messages.error(request, "Please enter a valid Order ID")
            return redirect('/TrackOrder')
        updates = TrackUpdate.objects.filter(order_id=order_id)
        if updates.count() == 0:
            messages.error(request, "No order found with this Order ID")
            return redirect('/TrackOrder')
...
def search(request):
    ...
    context = {
        'allProds': [[books, range(len(books))]],
        'search_query': query,
        'colleges': Book.objects.values_list('college', flat=True).distinct(),
